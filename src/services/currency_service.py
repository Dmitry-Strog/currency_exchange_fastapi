...
class CurrencyService:
    ...
    async def find_all_currency(self) -> list[CurrencySchemas]:
        currency_sequence = await self.currency_repo.find_all()
        return [
            CurrencySchemas.model_validate(currency) for currency in currency_sequence
        ]

    async def find_one_or_none_currency(self, currency_code: str) -> CurrencySchemas:
    ...
    async def create_one_currency(self, currency: InCurrencySchemas) -> CurrencySchemas:
        if not currency.sign or not currency.fullname:
            raise MissingFormField
        try:
            new_currency = await self.currency_repo.create_one(filters=currency)
